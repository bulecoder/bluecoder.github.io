...
* `__len__`（如果object没有定义__bool__方法，将其作为一个条件进行判断的时候，如果长度是0则为false）让类支持Python原生len()函数，数据集Dataset必须实现该方法（事实上自定义 PyTorch Dataset必须实现__len__、__getitem__和__int__三个方法）；
...
## Python 并发编程([学习视频地址](https://www.bilibili.com/video/BV1bK411A7tV?spm_id_from=333.788.videopod.episodes&vd_source=d3285a2ba86bc368a3901aac90d388ea))
...
### Python线程安全以及解决方案
1. 线程安全概念介绍  
线程安全指某个函数、函数库在多线程环境中被调用时，能够正确地处理多个线程之间的共享变量，使程序功能能正确完成。由于线程的执行随时会发生切换，就造成了不可预料的结果，出现了线程不安全(GIL规定同一时间只有一个线程能在CPU上执行Python字节码，同一时间可以多个线程访问共享资源）
...
### 好用的线程池 ThreadPoolExecutor
![alt text](figure/Python基础/线程池的原理.png)
![alt text](figure/Python基础/使用线程池的好处.png)
![alt text](figure/Python基础/ThreadPoolExecutor的使用语法.png)

#### 在Web服务中使用线程池加速
![alt text](figure/Python基础/Web服务的架构以及特点.png)
...
* 虽然有全局解释器锁GIL但是因为有IO的存在，多线程依然可以加速运行
* CPU密集型计算，现成的自动切换反而变成了负担，多线程甚至减慢了运行速度
* 多进程如何绕过GIL，每个进程有自己独立的CPython解释器和GIL，所以多个进程可以在多核CPU上真正并行执行，不受GIL限制，适合CPU密集型任务
* multiprocessing模块就是python为了解决GIL缺陷引入的一个模块，原理是用多进程在多CPU上并行执行
![alt text](figure/Python基础/多进程multiprocessing知识梳理.png)
