...
2. 文本到Cypher  
除了处理扁平化的元数据，查询构建技术还能应用于更复杂的数据结构，如图数据库。Cypher 是图数据库（如 Neo4j）中最常用的查询语言，其地位类似于 SQL 之于关系数据库。它采用一种直观的方式来匹配图中的模式和关系，例如 (:Person {name:"Tomaz"})-[:LIVES_IN]->(:Country {name:"Slovenia"}) 描述了一个人和一个国家以及他们之间的“居住在”关系。与“文本到元数据过滤器”类似，“文本到Cypher”技术利用大语言模型（LLM）将用户的自然语言问题直接翻译成一句精准的 Cypher 查询语句。

#### 文本到SQL
文本到SQL（Text-to-SQL）1 正是为了打破人与结构化数据之间的语言障碍而生。它利用大语言模型（LLM）将用户的自然语言问题，直接翻译成可以在数据库上执行的SQL查询语句。  
1. 业务挑战  
- 幻觉问题：LLM可能会想象出数据库中不存在的表或字段，导致生成的SQL语句无效
...
2. 优化策略
- 提供精确的数据库模式：向LLM提供数据库中相关表的 CREATE TABLE 语句
- 提供少量高质量的示例：在提示（Prompt）中加入一些“问题-SQL”的示例对，可以极大地提升LLM生成查询的准确性。
- 利用RAG增强上下文：为数据库构建一个专门的“知识库”，其中不仅包含DDL还包含表和字段的详细描述、同义词和业务术语、复杂的查询示例等
- 错误修正与反思：如果生成的SQL执行结果返回错误，将错误信息反馈给LLM进行修正

#### 查询重构与分发
这个阶段包含两个关键技术：查询翻译和查询路由  
1. 查询翻译  
目标是弥合用户自然语言提问与文档库中存储信息之间的“语义鸿沟”  
- 提示工程
- 多查询分解：将复杂问题拆分成多个简单具体的子问题，分别对子问题进行检索最后将检索结构合并去重
- 退步提示（Step-Back Promptin）：先从原始具体问题中生成一个更高层次更概括的退步问题，获取退步问题的答案，将通用原理作为上下文，结合原始的具体问题推理并生成最终答案
- 假设性文档嵌入（Hypothetical Document Embeddings, HyDE）：不直接使用用户的原始查询，而是先利用LLM来生成一个“假设性”的、能够完美回答该查询的文档。然后，HyDE 将这个内容详实的假设性文档进行向量化，用其生成的向量去数据库中寻找与之最相似的真实文档。

2. 查询路由（Query Routing）  
当系统接入了多个不同的数据源或具备多种处理能力时，就需要一个“智能调度中心”来分析用户的查询，并动态选择最合适的处理路径。其本质是替代硬编码规则，通过语义理解将查询分发至最匹配的数据源、处理组件或提示模板，从而提升系统的效率与答案的准确性。  
- 应用场景包括：数据源路由、组件路由和提示模板路由等
- 实现方式主要有基于LLM的意图识别（设计包含路由选项的提示词，让LLM直接对用户查询进行分类，输出代表路由选择的标签）和嵌入相似性路由（计算用户查询与预设的路由示例语句之间的向量嵌入相似度来做出决策）

#### 检索进阶
![alt text](figure/RAG与Agent/检索进阶在RAG中的位置.png)
1. 重排序（Re-ranking）  
- RRF（Reciprocal Rank Fusion）：它是一种简单而有效的零样本重排方法，不依赖于任何模型训练，而是纯粹基于文档在多个不同检索器（例如，一个稀疏检索器和一个密集检索器）结果列表中的排名来计算最终分数。
- RankLLM / LLM-based Reranker：这种方法通过一个精心设计的提示词来实现。该提示词会包含用户的查询和一系列候选文档（通常是文档的摘要或关键部分），然后要求 LLM 以特定格式（如 JSON）输出一个排序后的文档列表，并给出每个文档的相关性分数。
- Cross-Encoder 重排：Cross-Encoder（交叉编码器）能提供出色的重排精度。它的工作原理是将查询（Query）和每个候选文档（Document）拼接成一个单一的输入，然后将这个整体输入到一个预训练的 Transformer 模型中，模型最终会输出一个单一的分数，这个分数直接代表了文档与查询的相关性。
- ColBERT 重排：
...
### RAG系统评估
![alt text](figure/RAG与Agent/RAG评估三元组.png)
1. RAG评估三元组  
- 上下文相关性 (Context Relevance)：检索到的上下文内容，是否与用户的查询（Query）高度相关？
- 忠实度 / 可信度 (Faithfulness / Groundedness)：生成的答案是否完全基于所提供的上下文信息？
- 答案相关性 (Answer Relevance)：最终生成的答案是否直接、完整且有效地回答了用户的原始问题？

2. 评估工作流  
分为检索评估（上下文相关性）和响应评估（忠实度和答案相关性）
- 检索评估：需要一个标注数据集，使用精确率、召回率、F1分数、平均倒数排名、平均准确率均值等进行评估
- 响应评估：基于大语言模型评估和基于词汇重叠的经典指标


## Agent
### 一、智能体与语言模型基础
#### 大语言模型的缩放法则与局限性
- 缩放法则：研究发现，在对数-对数坐标系下，模型的性能（通常用损失 Loss 来衡量）与参数量、数据量和计算量这三个因素都呈现出平滑的幂律关系。简单来说，只要我们持续、按比例地增加这三个要素，模型的性能就会可预测地、平滑地提升，而不会出现明显的瓶颈。这一发现为大模型的设计和训练提供了清晰的指导：在资源允许的范围内，尽可能地扩大模型规模和训练数据量。
...
#### 基于低代码平台的智能体搭建
1. Coze  
以其零代码的友好体验和丰富的插件生态脱颖而出。通过"每日AI简报"案例，我们体验了如何通过拖拽式配置快速整合多源信息，并一键发布到多个主流平台。Coze 特别适合非技术背景用户和需要快速验证创意的场景，但其不支持 MCP 和无法导出标准化配置文件的局限性也值得注意。

2. Dify  
作为开源的企业级平台，展现了全栈式开发能力。"超级智能体个人助手"案例涵盖了日常问答、文案优化、多模态生成、数据分析和 MCP 工具集成等多个模块，充分展示了 Dify 在复杂业务场景下的强大编排能力。其丰富的插件市场(8000+)、灵活的部署方式和企业级安全特性，使其成为专业开发者和企业团队的理想选择。然而，相对陡峭的学习曲线和在高并发场景下的性能挑战也需要权衡。

3. n8n  
以其独特的"连接"能力开辟了另一条路径。通过"智能邮件助手"案例，我们看到了如何将 AI 能力无缝嵌入到复杂的业务自动化流程中。n8n 的 AI Agent 节点将模型、记忆和工具高度整合，配合其数百个预置节点，能够实现高度定制化的自动化方案。其支持私有化部署的特性对注重数据安全的企业尤为重要。但内置存储的非持久性和版本控制的不成熟，在生产环境中需要额外的工程化处理。

#### 框架开发实践
1. AutoGen 将复杂的协作抽象为一场由多角色参与的、可自动进行的“群聊”，其核心在于“以对话驱动协作”。
2. AgentScope 则着眼于工业级应用的健壮性与可扩展性，为构建高并发、分布式的多智能体系统提供了坚实的工程基础。
3. CAMEL 以其轻量级的“角色扮演”和“引导性提示”范式，展示了如何用最少的代码激发两个专家智能体之间深度、自主的协作。
4. LangGraph 则回归到更底层的“状态机”模型，通过显式的图结构赋予开发者对工作流的精确控制，尤其是其循环能力，为构建可反思、可修正的智能体铺平了道路。
...
- Host（用户界面）负责接收用户提问并与 模型交互，专注于用户体验
- Client（协议客户端），Client 负责与适当的 MCP Server 建立连接，发送请求并接收响应，专注于协议通信
- Server，具体功能实现  
...
![alt text](figure/RAG与Agent/MCP工作流程.png)
![alt text](<figure/RAG与Agent/MCP与Function calling.png>)

注意：MCPTool 主要用于 Stdio 和 Memory 传输
对于 StreamableHTTP 传输，建议使用底层的 MCPClient（SSE）
![alt text](figure/RAG与Agent/MCP传输方式.png)

2. A2A（Agent-to-Agent Protocol）：智能体间的对话  
A2A 协议采用点对点（P2P）架构（网状拓扑），允许智能体直接通信。为实现对协作过程的管理，A2A 为任务定义了标准化的生命周期，包括创建、协商、代理、执行中、完成、失败等状态。
...
![alt text](<figure/RAG与Agent/Agentic RL核心能力.png>)

![alt text](figure/RAG与Agent/LLM训练全景图.png)
