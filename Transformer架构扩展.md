...
$$SwiGLU(x) = (W_1 x) \odot \text{Swish}(W_2 x)$$
$$Swish(x) = x * sigmoid(x)$$
...
### 3. MoE（混合专家模型，Mixture of Experts）（[学习视频地址](https://www.bilibili.com/video/BV1CgZABxEcy/?spm_id_from=333.337.search-card.all.click&vd_source=d3285a2ba86bc368a3901aac90d388ea)）
**解决的核心问题**：打破「参数量越大，计算量越大」的魔咒，用更少的计算量实现更大的参数量和更好的效果。
#### 原理：
- 把原来的单个FFN层，拆分成N个独立的「专家网络」（每个专家都是一个独立的FFN），外加一个「路由器（Router）」（DeepSeekMoE多了一个共享专家，每次计算都选中共享专家，用来学习一些通用的知识）；
...
**核心优势**：
- 参数量可以做得很大，但实际计算量很小，训练和推理成本大幅降低（所有参数都需要加载都显存，所以显存占用并没有减少，但是参与计算的参数变少了，计算速度变快了）；
...
- YaRN（NTK-by-parts）：根本不应该插值高频维度，而应该只插值低频维度 。对高频维度保留原始 $\theta_d$，对低维频度应用$PI(\theta_d / s)$，在两者之间做平滑过渡。YaRN = “NTK-by-parts” 插值 + 注意力温度缩放。“NTK-by-parts” 修正了位置编码的频率，而“注意力缩放”修正了注意力分数的动态范围。
...
#### 具体实现
1. NTK-by-parts 频率插值：根据波长是否超过最大上下文长度，分三段处理
    - 波长极长维度（λ ≥ L） → 波长极长 → 插值（按缩放因子 s 缩小位置索引）
    - 长极短维度（λ ≤ L/k）→ 高频 → 不插值（完全外推）
    - 中间维度 → 平滑过渡
2. 注意力温度校正：引入温度参数 t（论文取 t = 0.1 ln(s) + 1）来调整 softmax 的输出分布，缓解注意力过度平均的问题（具体实现的时候直接在 RoPE编码矩阵中进行设计）
